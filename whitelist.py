...
def upsert_whitelist(
    account: str, kind: str, start_time: int, end_time: int, enabled: bool
):
    db = mongo.db()
...
def main(args):
    if hasattr(args, "filename"):
        ...
        (account, startTime, endTime, cloudType, enabled) = (
            args.username,
            args.start_time,
            args.end_time,
            args.type,
...
if __name__ == "__main__":
    ...
    manual_parser = subparsers.add_parser(
        "manual", description="insert account manually"
    )
    ...
    manual_parser.add_argument(
        ...
        type=str_to_bool,
        default=True,
        help="open or block permission [true/false]",
    )

    batch_parser = subparsers.add_parser(
        "batch", description="insert accounts in batch"
    )
